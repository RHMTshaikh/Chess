...
interface MovesPanelProps {
    ...
    indexToNotation: (Position:Position) => PositionNotation;
}

const MovesPanel: React.FC<MovesPanelProps> = ({ movesArray, undo, redo, currentStateFromServer }) => {
            ...
            <div className="moves">
                <ul>
                    {movesArray.current.map((move, index) => (
                        ...
                        <li key={index}>
                            <div className="number">{`${index+1}.`}</div><div className="from">{(move.from.position)}</div><div className="to">{(move.to.position)}</div> <div className={`piece piece-${move.to.piece<9 ? ('0'+move.to.piece):(move.to.piece)}`}></div>
                        </li>
                    ))}
                </ul>
            </div>
